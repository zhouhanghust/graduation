...
def load_data():
    x_data = np.load('../Chinese_ultimately/X.npy')[:2000,:]
    y_data = np.load('../Chinese_ultimately/y.npy')[:2000]
...
with tf.name_scope("train"):
    train_op = tf.train.AdadeltaOptimizer(1e-4).minimize(loss)
    ...
    for i in range(epoch):
        for j in range(batches):
            _,loss_,= sess.run([train_op,loss],\
                    feed_dict={wordsindex:X_train[j*batch_size:(j+1)*batch_size,:],labels:y_train[j*batch_size:(j+1)*batch_size]})
            summary_loss = sess.run(merge_summary_loss,feed_dict={wordsindex:X_test[:100,:],labels:y_test[:100]})
            summary_accuracy = sess.run(merge_summary_accuracy,feed_dict={wordsindex:X_test[:100,:],labels:y_test[:100]})
